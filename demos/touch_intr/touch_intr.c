...
void get_registers(i2c_dev *, uint8_t buf[16]);

/*
 * Read all 16 registers starting at 0 (this is touch and
 * gesture data)
 */
void
get_registers(i2c_dev *t, uint8_t buf[])
{
	buf[0] = 0;
	i2c_write(t, buf, 1, 0);
	i2c_read(t, buf, 16, 1);
...
/*
 * Tracks how many events and gestures the ISR processes
 */
...
exti9_5_isr(void)
{
	...
	/* fetch the registers */
	get_registers(touch_device, buf);
...
main(void)
{
	...
	/* Now set up the INT pin from the controller to generate interrupts */
	...
	/*
	 * Basic sanity check, make sure we see the FT6x06 device
	 * and print out other version information.
	 */
